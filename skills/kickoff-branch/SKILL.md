---
name: kickoff-branch
description: Create a git worktree and feature branch, then verify a clean test baseline before any code is written. Use when starting any non-trivial task, before implementing a feature or fix, to ensure isolation from main.
---
...
Suggest next skill: `develop-tdd` to start the TDD loop, or `execute-plan` if `the project release plan + epic capsule directories` already exists.
