---
name: verify-work
description: Multi-phase UAT gate - cold-start smoke, build, typecheck, lint, tests, manual step-through, gaps-closure. Use before declaring a feature or fix done, before requesting review, or when the user asks to verify work.
---
...
3. AGENTS.md preflight - before running default checks, call `bash the project's helper scripts` to detect project-specific commands. If `BP_PREFLIGHT` is set, run it instead of the default mechanical gates (or in addition to them if the project requires both). Output: `"Using preflight from AGENTS.md: <cmd>"`. Fall back to `CLAUDE.md` commands if AGENTS.md is absent.
