...
description: Dispatch multiple subagents in parallel on independent tasks. No waiting between them - all run concurrently. Use when there are several independent sub-tasks that can be researched or written in parallel, when the user says 'do these in parallel', or to reduce wall-clock time on multi-part work.
---
...
Before writing briefs, read `the project state file` if it exists - each agent gets only the decisions relevant to its task, nothing else.
