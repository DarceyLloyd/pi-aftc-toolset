...
const jiti = createJiti(__dirname, {
    // Resolve @earendil-works/* from the global pi install so type-only imports strip cleanly.
    alias: {
...
const tools = [
    ...
    { name: "skill_loader", description: "Load a skill", parameters: { type: "object" } },
];
let widgetSet = undefined;      // current widget factory (or string[] / undefined)
let widgetKey = null;
let widgetOpts = null;
...
const pi = {
    ...
    registerShortcut(name, opts) { shortcuts[name] = opts; },
    registerTool() {},
    ...
    setWidget(key, content, opts) {
        if (content === undefined) { widgetKey = null; widgetSet = undefined; widgetOpts = null; return; }
        widgetKey = key;
        widgetSet = content;
        widgetOpts = opts || null;
    },
    _setTools(t) { tools.length = 0; tools.push(...t); },
    _getWidget() { return widgetSet; },
    _getWidgetKey() { return widgetKey; },
    _getWidgetOpts() { return widgetOpts; },
};
...
function makeCtx() {
    return {
        ...
        ui: {
            ...
            select: async (title, lines, opts) => { console.log(`\n┌─ /${title} ─`); (lines || []).forEach(l => console.log("│ " + l)); console.log("└" + "─".repeat(title.length + 4)); return undefined; },
            confirm: async () => true,
        ...
        model: { name: "TestModel", id: "test-1", reasoning: true, contextWindow: 200000 },
    };
}
...
(async () => {
    ...
    const mod = jiti(path.resolve(__dirname, "../../extensions/toolset/index.ts"));
    ...
    // session_start
    await Promise.all((handlers.session_start || []).map(h => h({ reason: "startup" }, makeCtx())));
    console.log("session_start OK. Widget set:", pi._getWidgetKey() || "(none)", "placement:", pi._getWidgetOpts()?.placement);

    // before_agent_start
    await Promise.all((handlers.before_agent_start || []).map(h => h({ systemPrompt: "SYSTEM PROMPT v1" }, makeCtx())));

    // model_select + thinking_level_select
    await Promise.all((handlers.model_select || []).map(h => h({ model: { name: "M2", id: "m2", reasoning: false, contextWindow: 128000 } }, makeCtx())));
    await Promise.all((handlers.thinking_level_select || []).map(h => h({ level: "high", previousLevel: "off" }, makeCtx())));

    // message_end — turn 1 (cold, no cache)
    await Promise.all((handlers.message_start || []).map(h => h({
        message: { role: "user", content: "hi" },
    }, makeCtx())));
    await Promise.all((handlers.message_end || []).map(h => h({
        message: { role: "assistant", usage: { cacheRead: 0, cacheWrite: 5000, input: 5000, output: 300, totalTokens: 5300, cost: { total: 0.01 } } },
        model: { name: "M2", id: "m2" },
    }, makeCtx())));
    ...
    await Promise.all((handlers.message_end || []).map(h => h({
        message: { role: "assistant", usage: { cacheRead: 4800, cacheWrite: 200, input: 400, output: 350, totalTokens: 5750, cost: { total: 0.005 } } },
        model: { name: "M2", id: "m2" },
    }, makeCtx())));
    ...
    await Promise.all((handlers.message_end || []).map(h => h({
        message: { role: "assistant", usage: { cacheRead: 200, cacheWrite: 5200, input: 5000, output: 200, totalTokens: 10400, cost: { total: 0.02 } } },
        model: { name: "M2", id: "m2" },
    }, makeCtx())));
    ...
    // session_compact
    await Promise.all((handlers.session_compact || []).map(h => h({}, makeCtx())));
    ...
    if (pi._getWidgetKey() !== null) throw new Error("/aftc-footer did not clear the widget");
    console.log("--- /aftc-footer (toggle on) ---");
    await commands["aftc-footer"].handler("", makeCtx());
    if (pi._getWidgetKey() !== "aftc-cache") throw new Error("/aftc-footer did not re-show the widget");
    ...
    const dbPath = path.join(__dirname, "..", "..", ".pi-aftc-toolset", "data", "turns.db");
    ...
    const rows = db.prepare(`
        ...
        ORDER BY turn ASC
    `).all(tsBefore);
    if (rows.length !== 3) { throw new Error(`expected 3 recorded turns, got ${rows.length}`); }
    console.log(`OK found ${rows.length} recorded turns for this run:`);
    rows.forEach(r => console.log(`  turn=${r.turn} model=${r.model_name} think=${r.thinking_level} cost=$${r.cost_usd} in=${r.input_tokens} cached=${r.cache_read} user_prompt=${r.user_prompt} kind=${r.prompt_kind}`));
    // Each recorded turn should be a user prompt (the message_start for
    // user we fired earlier made them so).
    for (const r of rows) {
        if (r.user_prompt !== 1) throw new Error(`turn ${r.turn}: user_prompt=${r.user_prompt}, expected 1`);
        if (r.model_name !== "M2") throw new Error(`turn ${r.turn}: model_name=${r.model_name}, expected M2`);
        if (r.thinking_level !== "high") throw new Error(`turn ${r.turn}: thinking_level=${r.thinking_level}, expected high (we set it via thinking_level_select)`);
        if (r.cache_read < 0 || r.cache_write < 0) throw new Error(`turn ${r.turn}: negative cache tokens`);
        if (r.cost_usd <= 0) throw new Error(`turn ${r.turn}: cost_usd=${r.cost_usd}, expected > 0`);
    ...
    for (let i = 0; i < rows.length; i++) {
        if (Math.abs(rows[i].cost_usd - expectedCosts[i]) > 1e-9) {
            throw new Error(`turn ${i}: expected cost $${expectedCosts[i]}, got $${rows[i].cost_usd}`);
        }
    }
    console.log("OK all rows have correct model_name, thinking_level, user_prompt flag, cost signatures, and positive cost/cache tokens");
    db.close();
