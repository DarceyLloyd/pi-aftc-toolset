...
const { createUsageModule } = jiti(
	`${ROOT}/extensions/toolset/usage-report.ts`,
);
const fakePi = {
	registerCommand() {},
...
if (!data) {
	console.error("FAIL: collectReportData returned null");
	process.exit(1);
}
...
const checks = [
	[
		"data.totals is object",
		() => typeof data.totals === "object" && data.totals !== null,
	],
	[
		"data.sections.daily exists",
		() => !!data.sections && !!data.sections.daily,
	],
	[
		"data.sections.weekly exists",
		() => !!data.sections && !!data.sections.weekly,
	],
	[
		"data.sections.monthly exists",
		() => !!data.sections && !!data.sections.monthly,
	],
	[
		"data.sections.weeklyExcl exists",
		() => !!data.sections && !!data.sections.weeklyExcl,
	],
	[
		"data.sections.monthlyExcl exists",
		() => !!data.sections && !!data.sections.monthlyExcl,
	],
	[
		"data.modelsByPeriod has all periods",
		() =>
			!!data.modelsByPeriod &&
			["daily", "weekly", "monthly", "all"].every((p) =>
				Array.isArray(data.modelsByPeriod[p]),
			),
	],
	[
		"data.modelThinkingByPeriod has all periods",
		() =>
			!!data.modelThinkingByPeriod &&
			["daily", "weekly", "monthly", "all"].every((p) =>
				Array.isArray(data.modelThinkingByPeriod[p]),
			),
	],
	[
		"data.projections.rows is array",
		() => !!data.projections && Array.isArray(data.projections.rows),
	],
	[
		"data.projections.estimated is boolean",
		() => typeof data.projections.estimated === "boolean",
	],
	[
		"data.projections.note is string",
		() => typeof data.projections.note === "string",
	],
];
...
for (const k of sectionKeys) {
	...
	if (!s) {
		console.error("  FAIL: missing section " + k);
		shapeOk = false;
		continue;
	}
	if (typeof s.title !== "string" || typeof s.subtitle !== "string") {
		console.error("  FAIL: section " + k + " missing title/subtitle");
		shapeOk = false;
	}
	if (!Array.isArray(s.cards) || s.cards.length !== 4) {
		console.error(
			"  FAIL: section " +
				k +
				" expected 4 cards, got " +
				(s.cards?.length ?? "none"),
		);
		shapeOk = false;
		continue;
	...
	for (const c of s.cards) {
		if (
			...
			console.error("  FAIL: section " + k + " card missing fields");
			shapeOk = false;
		}
	}
...
if (projRows.length === 0 && rowCount > 0) {
	console.error("  FAIL: DB has rows but no projection rows generated");
	shapeOk = false;
}
const projFields = [
	"modelName",
...
for (const r of projRows) {
	for (const f of projFields) {
		if (!(f in r)) {
			console.error("  FAIL: projection row missing field " + f);
			shapeOk = false;
			break;
		}
	}
...
if (projRows.length)
	console.log(
		"  OK projection rows: " + projRows.length + " rows, all fields present",
	);
...
if (projRows.length)
	console.log("  OK projection math: costPerHour × 24 = costPerDay");
...
const requiredIds = [
	...
	"sec-weekly",
	"sec-monthly",
	"weekly-weekend-toggle",
	"monthly-weekend-toggle",
	"models-table",
	"models-tbody",
	"models-empty",
	"models-period",
...
if (!m2) {
	console.error("  FAIL: no module script");
	allOk = false;
} else {
	const stubs = `
        const _els = {};
