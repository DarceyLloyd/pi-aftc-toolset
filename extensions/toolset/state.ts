/**
 * pi-aftc-toolset — persistent state module.
 *
 * One file, one concern: `state.json` holds cross-session USER
 ...
 * three preferences actually changes (via `setPreference`) — never on a
 * timer, never on every turn, never on shutdown.
 *
 * All operations are best-effort. Errors are logged and the call falls
 * back to defaults rather than crashing pi. The state file is
 * gitignored (under .pi-aftc-toolset/) so it never leaks into the
 * repository.
 *
 ...
 *
 * See `state.readme.md` for the full contract.
 */

import * as fs from "node:fs";
import * as path from "node:path";
import { getDataDir, getStateJson } from "./paths";
 ...
 * Each field is optional on disk (so a partial or older state.json
 ...
 *   - generate a brand-new state.json on first access
 *     (`ensureStateFile`), and
...
// ─────────────────────────────────────────────────────────────────────────────
// Preferences (state.json) - ensure, read, write, cache
// ─────────────────────────────────────────────────────────────────────────────

let cachedPreferences: Preferences | null = null;

/**
 * Create state.json with `DEFAULT_PREFERENCES` if it doesn't exist
...
function ensureStateFile(): void {
    ...
    try {
        ...
        console.log(`[aftc-toolset] state.json ensure error: ${(err as Error).message}`);
    }
}
...
function loadPreferencesInternal(): Preferences {
    ...
    try {
        ...
        console.log(`[aftc-toolset] state.json read/parse error: ${(err as Error).message}`);
        cachedPreferences = { ...DEFAULT_PREFERENCES };
        return cachedPreferences;
    }
}
...
function savePreferencesInternal(prefs: Preferences): void {
    ...
    try {
        ...
        console.log(`[aftc-toolset] state.json write error: ${(err as Error).message}`);
    }
}
 ...
 * state.json atomically. Best-effort: errors are logged, never
 * thrown. This is the ONLY path that writes state.json after the
 * initial ensure — call it when footerEnabled / footerTimeframe /
 * responseDividerEnabled actually changes.
 */
