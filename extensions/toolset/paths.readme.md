...
`extensions/toolset/index.ts` underneath). Falls back to
...
`<package-root>/.pi-aftc-toolset/data/`. Holds `turns.db` and
...
`<package-root>/internal-python-gui/`. The bundled SSH GUI
(Python + PyQt6 + paramiko + Flask).
