...
NORMALIZE_DBFS = -1.0       # peak-normalize audio to this level
# ----------------------------------------------------------------------
...
def run_test(mp3s, scan_root: Path) -> int:
    ...
    if "--test" in args:
        ...
        if rest:
            candidate = (scan_root / rest[0]).resolve()
            if candidate.exists():
                target = candidate
    if target is None:
        target = mp3s[0]

    print(f"[test] processing single file: {target.relative_to(scan_root)}")
    ...
    out = scan_root / f"_test_{target.name}"
    out.write_bytes(mp3)
...
def main() -> int:
    ...
    if test_mode:
        return run_test(mp3s, scan_root)
