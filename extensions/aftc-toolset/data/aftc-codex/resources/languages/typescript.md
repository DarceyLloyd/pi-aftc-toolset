...
- [7kE0u2] A regex inside JS/CSS embedded in a TS template literal silently loses its backslashes (\d -> d), so patterns like a thousands-separator never match at runtime
  Cause: template literals interpret escape sequences at build time, so \B/\d/\s are unescaped before the client code ever runs; the mangled regex is still syntactically valid, so `node --check` and syntax tests pass while behaviour is broken.
  Fix: never author embedded client code inside a template literal — keep it in real files (readFileSync + concatenation copies bytes verbatim); if unavoidable, double every backslash and add a test that greps the GENERATED output for the exact escaped pattern. (2026-08)
